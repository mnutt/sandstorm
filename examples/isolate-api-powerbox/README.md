...
`api.storage().put(...)`. The URL and scopes are editable; the browser uses the
native client and the worker sends claims through the typed supervisor bridge,
so no private HTTP control routes are involved.
