...
# Clear bundle directory (use rm contents if it's a mount point, otherwise rm -rf)
if mountpoint -q bundle 2>/dev/null; then
  rm -rf bundle/* bundle/.[!.]*
else
  rm -rf bundle
fi
...
# Use /. suffix to copy contents when bundle is a mount point
...
# Both versions are bundled to allow gradual migration - users run 'sandstorm migrate-mongo'
