...
// Trusted runtime adapter for a host-minted, same-process ArrayBuffer link. The host transfers the
// backing store between V8 isolates and validates each complete RPC frame before delivery, so this
// transport must preserve one sendMessage() call as exactly one channel.send() call.
export class NativeCapnpLocalBufferTransport extends CapnpEsDeferredTransport {
  #channel;
  #connection;
  ...
  constructor(channel, options = {}) {
    ...
    if (!channel || typeof channel.send !== "function" ||
        typeof channel.receive !== "function" || typeof channel.close !== "function") {
      throw new TypeError(
        "NativeCapnpLocalBufferTransport requires a native local buffer channel");
    }

    this.#channel = channel;
    this.#connection = options.connection || null;
    this.kind = "nativeLocalBuffer";
    this.#readLoop();
  }
  ...
  sendMessage(message) {
    if (this.closed) {
      throw new CapnpUnavailableError(
        "native Cap'n Proto local buffer transport is closed");
    }
  ...
  async #readLoop() {
    try {
      while (!this.closed) {
        const buffer = await this.#channel.receive();
        this.resolve(nativeCapnpMessageBytes(buffer));
      }
    } catch (error) {
      this.abort(error);
    }
