...
private:
  ...
  static kj::String generateDevIsolateCapnpModule(
    ...
    output.addAll(kj::StringPtr(
        ...
        "  return target;\n"
        "}\n\n"
        "function makeClient(interfaceName, methodNames, caller, extras = {}) {\n"
        "  const client = { ...extras };\n"
        "  for (const methodName of methodNames) {\n"
    ...
    output.addAll(kj::StringPtr(
        ...
        "    schemaPath,\n"
        "    methodNames,\n"
        "    implement(methods) { return makeServerTarget(interfaceName, methodNames, methods); },\n"
        "    cast(capability) {\n"
        ...
        "      }\n"
        "      return makeClient(interfaceName, methodNames,\n"
        ...
        "    local(methods) {\n"
        "      const source = requiredMethods(interfaceName, methods);\n"
        "      return makeClient(interfaceName, methodNames, async (methodName, args) => {\n"
        "        const method = source[methodName];\n"
        "        if (typeof method !== \"function\") {\n"
        "          throw new TypeError(`${interfaceName}.${methodName} is not implemented`);\n"
        "        }\n"
